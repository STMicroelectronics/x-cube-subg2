...
/**
 * @brief  M2S Exti Line array, Added for STM32CubeMX integration
 */
static const uint32_t s_vectcM2SGpioExtiLine[4] = {
...
void S2LP_Middleware_GpioTriggerRising(M2SGpioPin xGpio, uint8_t enable)
{
  ...
  exti.Line = s_vectcM2SGpioExtiLine[xGpio];
  HAL_EXTI_GetConfigLine(&exti, &exticfg);
  if(enable){
	exticfg.Trigger |= EXTI_TRIGGER_RISING;
	HAL_EXTI_SetConfigLine(&exti, &exticfg);
  } else {
	exticfg.Trigger &= ~EXTI_TRIGGER_RISING;
	HAL_EXTI_SetConfigLine(&exti, &exticfg);
  }
...
uint8_t S2LP_Middleware_GetTriggerRising(M2SGpioPin xGpio)
{
  ...
  exti.Line = s_vectcM2SGpioExtiLine[xGpio];
  HAL_EXTI_GetConfigLine(&exti, &exticfg);
  if (exticfg.Trigger & EXTI_TRIGGER_RISING) {
	  return 1;
  }
  return 0;
}
...
void S2LP_Middleware_GpioTriggerFalling(M2SGpioPin xGpio, uint8_t enable)
{
  ...
  HAL_EXTI_GetConfigLine(&exti, &exticfg);
  if(enable){
	exticfg.Trigger |= EXTI_TRIGGER_FALLING;
	HAL_EXTI_SetConfigLine(&exti, &exticfg);
  } else {
	exticfg.Trigger &= ~EXTI_TRIGGER_FALLING;
	HAL_EXTI_SetConfigLine(&exti, &exticfg);
...
uint8_t S2LP_Middleware_GpioGetTriggerFalling(M2SGpioPin xGpio)
{
  ...
  exti.Line = s_vectcM2SGpioExtiLine[xGpio];
  HAL_EXTI_GetConfigLine(&exti, &exticfg);
  if (exticfg.Trigger & EXTI_TRIGGER_FALLING) {
	  return 1;
  }
  return 0;
}
